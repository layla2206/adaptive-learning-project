...
export async function GET(req: NextRequest) {
  ...
  try {
    const response = await fetch(`${FASTAPI_URL}/peer-buddy/history`, {
      method: "POST",
    ...
    if (!response.ok) {
      return NextResponse.json({ error: data.detail ?? data.error ?? "Unable to load peer buddy history" }, { status: response.status });
    }
    return NextResponse.json(data);
  } catch (error) {
    console.error("Peer buddy history proxy error:", error);
